 ...
 * Proper CSS layout, real progress bars, responsive grid.
...
function Stat({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <span className={`text-xs sm:text-sm ${className}`}>{children}</span>;
}

function Separator() {
  return <span className="text-green-900 mx-1 select-none">·</span>;
}
    ...
    <div className="space-y-1">
      <div className="flex justify-between text-xs sm:text-sm">
        ...
        <span className="text-green-600 shrink-0">
          Lv{tree.level}  {p}%  {tree.xp}/{tree.xpToNextLevel}
        </span>
      </div>
      <ProgressBar value={p} />
    </div>
...
export function DashboardTerminal({ data }: { data: DashboardPayload }) {
  ...
  const hasEffects = data.statusEffects.length > 0;
    ...
    <div className="flex flex-col min-h-screen bg-black text-green-400/90 font-mono">
      ...
      <header className="border-b border-green-900 px-3 sm:px-5 py-2 flex items-center justify-between gap-3">
        ...
        <form action="/api/auth/sign-out" method="post" className="shrink-0">
          <button
            type="submit"
            className="text-green-800 hover:text-green-400 text-xs sm:text-sm transition-colors"
          >
            sign_out
          </button>
        </form>
      ...
      <main className="flex-1 overflow-y-auto px-3 sm:px-5 py-4 space-y-5">
        {/* Focus signal */}
        {data.focusSignal && (
          <div className="text-green-400 border-l-2 border-green-600 pl-3 py-1 text-sm flex items-center gap-2">
            <span className="text-green-600">⚡</span>
            <span>{data.focusSignal.message}</span>
          </div>
        )}

        {/* Same-day reminder */}
        {!data.loggedToday && (
          <div className="text-yellow-600 border-l-2 border-yellow-700 pl-3 py-1 text-sm flex items-center gap-2">
            <span className="text-yellow-700">!</span>
            <span>Nothing logged today yet — a quick entry keeps your story up to date.</span>
          </div>
        )}

        {/* ── Core trees ── */}
        <section>
          <h2 className="text-green-800 text-xs tracking-widest mb-3 border-b border-green-900 pb-1">
            // CORE
          </h2>
        ...
        <section>
          ...
          {data.quests.length === 0 ? (
            <div className="text-green-800 text-xs">No active quests. Create one to start earning XP.</div>
          ) : (
            <ul className="space-y-1">
              {data.quests.map(q => (
                <li key={q.id} className="flex items-baseline gap-2 text-xs sm:text-sm">
                  <span className="text-green-600 w-4 shrink-0">
                    {q.status === 'ACTIVE' ? '▶' : '○'}
                  </span>
        ...
        <section className="border border-green-900 px-3 sm:px-4 py-3">
          <h3 className="text-green-800 text-xs tracking-widest mb-2">
            // DAILY BRIEFING
          </h3>
          <TypewriterBox
      ...
      {/* ── footer ── */}
      <footer className="border-t border-green-900 px-3 sm:px-5 py-1.5 text-green-800 text-[10px] flex items-center justify-between">
        <span>{`[${'='.repeat(16)}]`}</span>
        <span>v0.3</span>
      </footer>
    </div>
